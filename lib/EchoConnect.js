...
/**
 * EchoConnect provides a high-level interface for interacting with Amazon Alexa devices
 * using the alexa-remote library. It supports device management, sending commands,
 * controlling playback, managing notifications, handling authentication, and push connections.
 * 
 * EchoConnect emits events for device and connection status changes, and wraps most
 * Alexa operations in Promise-based methods for easy async/await usage.
 * 
 * @class EchoConnect
 * @extends EventEmitter
 * 
 * @param {boolean} [showLog=false] - Enables verbose logging if true.
 * @param {string} [installationId=''] - Unique identifier for this installation.
 * 
 * @fires cookieGenerated - Emitted when authentication cookie is generated or refreshed.
 * @fires pushConnected - Emitted when push connection to Alexa is established.
 * @fires pushDisconnected - Emitted when push connection to Alexa is lost or stopped.
 * @fires volumeChanged - Emitted when device volume changes.
 * @fires playerChanged - Emitted when player state changes.
 * @fires queueChanged - Emitted when media queue changes.
 * @fires deviceActivity - Emitted when a device responds to a voice command.
 * @fires alexaConnected - Emitted when Alexa initialization succeeds.
 * @fires alexaDisconnected - Emitted when Alexa initialization fails.
 * 
 * @example
 * const echoConnect = new EchoConnect(true, 'installation-id');
 * echoConnect.on('alexaConnected', devices => {
 *   console.log('Connected to Alexa, devices:', devices);
 * });
 * await echoConnect.initAlexa({ cookieData: '...', amazonPage: 'amazon.de' });
 */
class EchoConnect extends EventEmitter {
  /**
   * Creates a new EchoConnect instance.
   * @param {boolean} [showLog=false] - Enables verbose logging if true.
   * @param {string} [installationId=''] - Unique identifier for this installation.
   * @param {FileLogger|null} [fileLogger=null] - Optional FileLogger instance for persistent logging.
   */
  constructor(showLog = false, installationId = '', fileLogger = null) {
    super();
  ...
  /**
   * Private method that waits for Alexa service initialization.
   * 
   * @private
  ...
  async _waitForInit() {
    ...
    if (!this._isInitializing && !this._initPromise) {
      return Promise.reject(new AlexaError('ERROR_INIT', 'Alexa initialization is not running'));
    }
  ...
  /**
   * Logs messages to the console (if showLog is true) and to file (if fileLogger is available).
   * Console logging and file logging are independent settings.
   *
   * @private
   * 
   ...
   */
  _log(...args) {
    // Console logging: only if showLog is true
    if (this.showLog) {
      const timestamp = new Date().toISOString();
      console.log(`%c${timestamp}`, 'color: green', `[ECHO-CONNECT] -`, ...args);
    }
    
    // File logging: independent of showLog, only requires fileLogger
    if (this.fileLogger) {
      const message = args.map(arg => 
        typeof arg === 'object' ? JSON.stringify(arg) : String(arg)
      ).join(' ');
      this.fileLogger.info('ECHO-CONNECT', message);
    }
  }

  /**
   * Logs error messages to the console.
   * Also writes to FileLogger if available (always, regardless of showLog setting).
   *
   * @private
   * 
   * @param {...any} args - The arguments to log.
  ...
  _error(...args) {
    const timestamp = new Date().toISOString();
    console.error(`${timestamp}`, `[ECHO-CONNECT-ERROR] -`, ...args);
    
    // Write to file logger if available (errors are always logged to file)
    if (this.fileLogger) {
      const message = args.map(arg => 
        typeof arg === 'object' ? JSON.stringify(arg) : String(arg)
      ).join(' ');
      this.fileLogger.error('ECHO-CONNECT', message);
    }
  }

  _getDeviceAppName() {
    if (typeof this.installationId === 'string' && this.installationId.trim().length > 0) {
      return `${CONFIG.APP_NAME}-${this.installationId}`;
    }
  ...
  _registerAlexaListeners() {
    this.alexa.on('command', (command) => {
      // Raw Push Command logging removed - too verbose for production use
      // Enable temporarily for debugging: this._log('[DEBUG] Raw Push Command:', JSON.stringify(command, null, 2));
    });
    ...
    this.alexa.on('ws-volume-change', async (payload) => {
      ...
      try {
        ...
        if (echoExist) {
          ...
          this._log('[WARN] Volume change received for unmanaged device:', echoSerial);
        }
      } catch (error) {
        // Network errors are temporary and auto-recover, log as warning
        if (error.message?.includes('ENOTFOUND') || error.message?.includes('getaddrinfo')) {
          this._log('[WARN] Temporary network error in ws-volume-change:', error.message);
        } else {
          this._error('error in ws-volume-change:', error);
        }
      }
    });
    ...
    this.alexa.on('ws-audio-player-state-change', async (payload) => {
      ...
      try {
        ...
        this._error("Error retrieving player information:", error);
      }
    });
    ...
    this.alexa.on('ws-device-activity', (activity) => {
      this._log('Device Activity called')

      // Filter only the device that actually responded
      if (activity.data && activity.data.utteranceType === 'GENERAL') {
        this._log(`Device Activity - Responding device: ${activity.name} (${activity.deviceSerialNumber})`);
        this._log(`Command: "${activity.description?.summary || 'N/A'}"`);
        this._log(`Response: "${activity.alexaResponse || 'N/A'}"`);

        // Emit only for the device that responded
        this.emit('deviceActivity', {
          deviceSerial: activity.deviceSerialNumber,
          deviceName: activity.name,
          deviceType: activity.deviceType,
          command: activity.description?.summary || '',
          response: activity.alexaResponse || '',
          domain: activity.data.domain || '',
          intent: activity.data.intent || '',
          timestamp: activity.creationTimestamp,
          destinationUserId: activity.destinationUserId
        });
      } else {
        // Log for other devices that heard but did not respond
        this._log(`Device Activity - Ignored: ${activity.name} (${activity.data?.utteranceType || 'UNKNOWN'}) - Not the responding device`);
      }
    });

    // Listener when alexa is connected - **NOT WORKING**
    this.alexa.on('ws-device-connection-change', (payload) => {
      this._log('Device connection change:', payload);
  ...
  _getAlexaConfig(cookieData, page, forceRefresh, message, imageUrl) {
    this._log('getAlexaConfig called - cookieData present:', !this.isCookieEmptyOrNull(cookieData));
  ...
  speakEcho(device, message, type = COMMAND_TYPES.SPEAK) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Check message length (Alexa limit is 250 characters)
        if (!message || typeof message !== 'string') {
          this._error('speakEcho', 'Message must be a non-empty string');
          reject(new AlexaError('INVALID_MESSAGE', 'Message must be a non-empty string'));
          return;
        }

        if (message.length > 250) {
          this._error('speakEcho', `Text too long: ${message.length} characters (limit: 250)`);
          reject(new AlexaError('TEXT_TOO_LONG', `Text too long, limit is 250 characters (current: ${message.length})`));
          return;
        }
        ...
        this.alexa.sendSequenceCommand(validDevice, command, value, (error, result) => {
          if (error) {
            this._error('speakEcho', `Error sending speak command: ${error.message || error}`, error);
        ...
        this._error('speakEcho', `Validation error: ${validationError.message}`, validationError);
  ...
  async executeAlexaCommand(serial, command) {
    ...
    return new Promise((resolve, reject) => {
      this.alexa.sendSequenceCommand(serial, 'textCommand', command, (error, result) => {
        if (error) {
          this._error('executeAlexaCommand', `Error sending text command: ${error.message || error}`, error);
   ...
   */
  async changePlayback(serial, command, value = true) {
    ...
    return new Promise((resolve, reject) => {
      this.alexa.sendCommand(serial, command, value, (error, result) => {
        if (error) {
          this._error('Error sending command:', error);
          ...
          if (result.message === null && result.userFacingMessage === null) {
            this._error('changePlayback', 'Function not supported by your Echo device');
   ...
   */
  async getAudioGroupsList() {
    await this._waitForInit();
    return new Promise((resolve, reject) => {
      this.alexa.getWholeHomeAudioGroups((error, groups) => {
        if (error) {
          this._error('getAudioGroupsList', `Error retrieving audio groups: ${error.message || error}`, error);
   ...
   */
  async getRoutinesList(limit = CONFIG.DEFAULT_ROUTINES_LIMIT) {
    await this._waitForInit();
    return new Promise((resolve, reject) => {
      this.alexa.getAutomationRoutines(limit, (error, result) => {
        if (error) {
          this._error('getRoutinesList', `Error retrieving routines: ${error.message || error}`, error);
   ...
   */
  async executeAutomationRoutine(serial, routine) {
    await this._waitForInit();
    return new Promise((resolve, reject) => {
      this.alexa.executeAutomationRoutine(serial, routine, (error, result) => {
        if (error) {
          this._error('executeAutomationRoutine', `Error executing automation routine: ${error.message || error}`, error);
          reject(new AlexaError('ERROR_ROUTINE', error));
        } else {
          resolve(result);
        }
  ...
  /**
   * Retrieves the list of available sounds that can be used in Alexa routines.
   * Returns a simplified list containing only the sound ID and name for each sound.
   *
   * @returns {Promise<Array<Object>>} A promise that resolves with an array of simplified sound objects.
   * @throws {AlexaError} If there's an error retrieving the sound list.
   *
   * @example
   * try {
   *   const sounds = await echoConnect.getRoutineSoundList();
   *   console.log('Available routine sounds:', sounds);
   *   // Output example:
   *   // [
   *   //   { id: 'amzn1.ask.1p.sound.soundStringId1', name: 'Chime' },
   *   //   { id: 'amzn1.ask.1p.sound.soundStringId2', name: 'Bell' },
   *   //   ...
   *   // ]
   * } catch (error) {
   *   console.error('Error retrieving routine sounds:', error.message);
   * }
   */
  async getRoutineSoundList() {
    await this._waitForInit();
    return new Promise((resolve, reject) => {
      if (!this.alexa || typeof this.alexa.getRoutineSoundList !== 'function') {
        this._error('getRoutineSoundList', 'getRoutineSoundList method not available on alexa-remote instance');
        reject(new AlexaError('ERROR_ROUTINE_SOUNDS', 'getRoutineSoundList method not available on alexa-remote instance'));
        return;
      }

      this.alexa.getRoutineSoundList((error, soundList) => {
        if (error) {
          this._error('getRoutineSoundList', `Failed to retrieve routine sound list: ${error.message || error}`, error);
          reject(new AlexaError('ERROR_ROUTINE_SOUNDS', `Failed to retrieve routine sound list: ${error.message || error}`, error));
        } else {
          try {
            // Simplify the sound list to contain only id and name
            const simplifiedSoundList = (soundList || []).map(sound => ({
              id: sound.id || sound.soundStringId,
              name: sound.name || sound.displayName || 'Unknown'
            }));

            this._log(`Retrieved ${simplifiedSoundList.length} routine sounds`);
            resolve(simplifiedSoundList);
          } catch (mappingError) {
            this._error('getRoutineSoundList', `Failed to process sound list: ${mappingError.message}`, mappingError);
            reject(new AlexaError('ERROR_ROUTINE_SOUNDS', `Failed to process sound list: ${mappingError.message}`, mappingError));
          }
        }
      });
    });
  ...
  /**
   * Plays a routine sound on the specified Alexa device.
   * The sound must be one of the available sounds from getRoutineSoundList().
   *
   * @param {string} deviceSerial - The serial number of the Alexa device.
   * @param {string} soundId - The ID of the sound to play (from getRoutineSoundList).
   * @returns {Promise<any>} A promise that resolves with the result of the sound playback.
   * @throws {AlexaError} If there's an error playing the sound.
   *
   * @example
   * try {
   ...
   *   const sounds = await echoConnect.getRoutineSoundList();
   *   
   *   // Play the first sound on the device
   *   if (sounds.length > 0) {
   *     const result = await echoConnect.playRoutineSound('G090LF09643202VS', sounds[0].id);
   *     console.log('Sound playback initiated:', result);
   *   }
   * } catch (error) {
   *   console.error('Error playing routine sound:', error.message);
   * }
   */
  async playRoutineSound(deviceSerial, soundId) {
    await this._waitForInit();
    return new Promise((resolve, reject) => {
      try {
        const validSerial = this._validateDeviceSerial(deviceSerial);

        if (!soundId || typeof soundId !== 'string' || soundId.trim().length === 0) {
          this._error('playRoutineSound', 'Sound ID is required and must be a non-empty string');
          reject(new AlexaError('INVALID_SOUND_ID', 'Sound ID is required and must be a non-empty string'));
          return;
        }

        if (!this.alexa || typeof this.alexa.sendSequenceCommand !== 'function') {
          this._error('playRoutineSound', 'sendSequenceCommand method not available on alexa-remote instance');
          reject(new AlexaError('ERROR_SOUND_PLAYBACK', 'sendSequenceCommand method not available on alexa-remote instance'));
          return;
        }

        this._log(`Sending sequence command 'sound' to device ${validSerial} with sound ID: ${soundId}`);

        // Use sendSequenceCommand with 'sound' command type
        this.alexa.sendSequenceCommand(validSerial, 'sound', soundId.trim(), (error, result) => {
          if (error) {
            this._error('playRoutineSound', `Failed to play routine sound: ${error.message || error}`, error);
            reject(new AlexaError('ERROR_SOUND_PLAYBACK', `Failed to play routine sound: ${error.message || error}`, error));
          } else {
            this._log(`Sound playback initiated successfully on device ${validSerial}`);
            resolve(result);
          }
        });
      } catch (validationError) {
        this._error('playRoutineSound', `Validation error: ${validationError.message}`, validationError);
        reject(validationError);
      }
    });
  }
  ...
  async getPlayerInfo(serial) {
    ...
    return new Promise((resolve, reject) => {
      this.alexa.getPlayerInfo(serial, (error, playerInfo) => {
        if (error) {
          this._error('getPlayerInfo', `Error retrieving player info: ${error.message || error}`, error);
   ...
   */
  async createAlexaNotification(serial, type, label, value, status = "ON") {
    await this._waitForInit();
    return new Promise((resolve, reject) => {
      ...
      if (!notification) {
        this._error('createAlexaNotification', 'Unable to create notification object');
      ...
      this.alexa.createNotification(notification, (error, response) => {
        if (error) {
          this._error('createAlexaNotification', `Error creating notification: ${error.message || error}`, error);
          ...
          if (response.hasOwnProperty('message') &&
            ...
            this._error('createAlexaNotification', `Error pushing notification: Error: ${err}`);
  ...
  async setVolumeDevice(serial, volume) {
    ...
    return new Promise((resolve, reject) => {
      this.alexa.sendSequenceCommand(validSerial, 'volume', validVolume, null, (error, result) => {
        if (error) {
          this._error('setVolumeDevice', `Error setting volume: ${error.message || error}`, error);
   ...
   */
  async getVolumeDevice(serial) {
    await this._waitForInit();
    return new Promise((resolve, reject) => {
      this.alexa.getAllDeviceVolumes((err, state) => {
        if (err) {
          this._error('getVolumeDevice', `Error recovering device volume: ${err.message}`);
        ...
        if (echo.length > 0) {
          ...
          // Device not found is often temporary after reconnection, log as warning
          this._log('[WARN] getVolumeDevice - Serial number not found (may be syncing):', serial);
   ...
   */
  async setDisplayPowerSetting(serial, enabled) {
    const validSerial = this._validateDeviceSerial(serial);

    if (typeof enabled !== 'boolean') {
      this._error('setDisplayPowerSetting', 'Enabled setting must be a boolean');
    ...
    return new Promise((resolve, reject) => {
      ...
      this.alexa.setDisplayPowerSetting(validSerial, enabled, (error, result) => {
        if (error) {
          this._error('setDisplayPowerSetting', `Error setting display power: ${error.message || error}`, error);
   ...
   */
  async getDisplayPowerSetting(serial) {
    const validSerial = this._validateDeviceSerial(serial);
    ...
    return new Promise((resolve, reject) => {
      ...
      this.alexa.getDisplayPowerSetting(validSerial, (error, result) => {
        if (error) {
          this._error('getDisplayPowerSetting', `Error getting display power settings: ${error.message || error}`, error);
  ...
  /**
   * Actively verifies the push connection with HTTP/2 ping.
   * More reliable than isPushConnected() but slower (~100ms).
   * Use this when you need absolute certainty about the connection status.
   *
   * @returns {Promise<boolean>} A promise that resolves to true if the push connection is active and responsive, false otherwise.
   * @throws {AlexaError} If there's an error verifying the connection.
   *
   * @example
   * try {
   *   const isConnected = await echoConnect.verifyPushConnection();
   *   console.log('Push connection verified:', isConnected ? 'Connected' : 'Not connected');
   * } catch (error) {
   *   console.error('Verification failed:', error);
   * }
   */
  async verifyPushConnectionWithPing() {
    await this._waitForInit();
    return new Promise((resolve, reject) => {
      if (!this.alexa || typeof this.alexa.verifyPushConnection !== 'function') {
        this._error('verifyPushConnectionWithPing', 'verifyPushConnection method not available on alexa-remote instance');
        reject(new AlexaError('ERROR_VERIFY_PUSH', 'verifyPushConnection method not available on alexa-remote instance'));
        return;
      }

      this.alexa.verifyPushConnection((isConnected) => {
        this._log(`verifyPushConnection result: ${isConnected}`);
        resolve(isConnected);
      });
    });
  }
  ...
  async isAuthenticated() {
    // Note: Do not use _waitForInit here as isAuthenticated is part of the initialization flow itself
    return new Promise((resolve, reject) => {
      this.alexa.checkAuthentication((authenticated, err) => {
        if (err) {
          this._error('checkAuthentication', 'Error while verifying authentication', err.message || String(err));
   ...
   */
  async checkAuthenticationAndPush(cookieData = this.alexa.cookieData, amazonPage = this.amazonPage) {
    this._log('checkAuthenticationAndPush called - cookieData present:', !this.isCookieEmptyOrNull(cookieData));

    // Ensure _initPromise exists
    if (!this._initPromise) {
      this._initPromise = new Promise((resolve, reject) => {
        this._resolveInit = resolve;
        this._rejectInit = reject;
    ...
    if (this.isCookieEmptyOrNull(cookieData)) {
      const error = new AlexaError('ERROR_AUTHENTICATION', 'Cookie data is empty or null');
    ...
    try {
      ...
      if (authenticated) {
        // If authentication is valid, resolve the init promise
        ...
        if (this.isPushConnected()) {
          ...
          try {
            await this.initPushMessage();
            return true; // Return true after push initializing
          } catch (error) {
            this._error('Error during push service initializing:', error);
        ...
        try {
          await this.initAlexa({
            cookieData: cookieData,
            amazonPage: amazonPage
          });
          return true; // Initialization is successful
        } catch (error) {
          this._error('Error during Alexa initializing:', error?.message);
          this.emit('alexaDisconnected');
          return false; // Authentication is definitely not valid - need a new login
        }
      }
    } catch (err) {
      this._error('Error during authentication check:', err);
      const error = new AlexaError('ERROR_AUTHENTICATION', err);
      if (this._rejectInit) this._rejectInit(error);
      if (this._initPromise) {
        this._initPromise.catch(() => {}); // Prevent unhandled rejection
        this._initPromise = null;
    ...
    return this.getDevices()
      .then(allDevices => {
        ...
        if (device) {
          ...
          this._error("Device not found.");
          return null;
        }
      })
      .catch(error => {
        const alexaError = new AlexaError('ERROR_FIND_DEVICE', `Failed to find device ${serial}: ${error.message || error}`, error);
        this._error('Error during device search:', alexaError.message);
        throw alexaError;
      });
  }
  ...
  isOnLine(serial) {
    return new Promise((resolve, reject) => {
      this.findDevice(serial)
        ...
        .catch(err => {
          reject(new AlexaError('ERROR_ONLINE_CHECK', `Failed to check device online status: ${err.message || err}`, err));
        });
    });
  }
  ...
  async getDevices() {
    ...
    return new Promise((resolve, reject) => {
      this.alexa.getDevices((err, devices) => {
        if (err) {
          reject(new AlexaError('ERROR_DEVICES', `Failed to retrieve devices: ${err.message || err}`, err));
        } else {
          resolve(devices);
        }
      });
  ...
  async initAlexa({
    ...
    if (this._isInitialized) {
      return Promise.resolve(this.echoDevices);
    }

    // If already initializing, return the existing promise
    if (this._initPromise && this._isInitializing) {
      return this._initPromise;
    }

    this._isInitializing = true;

    // Set initialization promise if not already present
    if (!this._initPromise) {
      this._initPromise = new Promise((resolve, reject) => {
        this._resolveInit = resolve;
        this._rejectInit = reject;
    ...
    if (retryCount < 0 || maxRetries < 0) {
      this._error('initAlexa', 'Retry counts must be non-negative numbers');
  ...
  async _handleInitError(err, options, resolve, reject) {
    this._error('Error during Alexa initialization:', err?.message);
    ...
    if (cookieExist && options.retryCount < options.maxRetries) {
      ...
      const error = new AlexaError('ERROR_INIT', 'Unable to initialize Alexa after multiple attempts', err);
      this._isInitializing = false;
      if (this._rejectInit) this._rejectInit(error);
      if (this._initPromise) {
        this._initPromise.catch(() => {}); // Prevent unhandled rejection
  ...
  /**
   * Handles the case when a new login is required
   * @private
   */
  ...
  async _handleInitSuccess(resolve, reject) {
    ...
    try {
      ...
      this._isInitialized = true;
      this._isInitializing = false;
      if (this._resolveInit) this._resolveInit(echoDevices);

      this.emit('alexaConnected', echoDevices);
      resolve(echoDevices);
    } catch (error) {
      this._error('Error recovering devices:', error);
      const alexaError = new AlexaError('ERROR_INIT', 'Failed to recover devices', error);
      this._isInitializing = false;
      if (this._rejectInit) this._rejectInit(alexaError);
      if (this._initPromise) {
        this._initPromise.catch(() => {}); // Prevent unhandled rejection
        this._initPromise = null;
      }
  ...
  _validateDeviceSerial(serial) {
    if (!serial || typeof serial !== 'string' || serial.trim().length === 0) {
      this._error('_validateDeviceSerial', 'Device serial number is required and must be a non-empty string');
  ...
  _validateVolumeLevel(volume) {
    ...
    if (isNaN(numVolume) || numVolume < 0 || numVolume > 100) {
      this._error('_validateVolumeLevel', 'Volume must be a number between 0 and 100');
