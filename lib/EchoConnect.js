...
class EchoConnect extends EventEmitter {
  constructor(showLog = false, installationId = '') {
    super();
  ...
  _registerAlexaListeners() {
    this.alexa.on('command', (command) => {
      this._log('[DEBUG] Raw Push Command Received:', JSON.stringify(command, null, 2));
    });
   ...
   * }
   */
  playRoutineSound(deviceSerial, soundId) {
    return new Promise((resolve, reject) => {
      try {
  ...
  findDevice(serial) {
    return this.getDevices()
      ...
      .catch(error => {
        ...
        console.error('Error during device search:', alexaError.message);
        throw alexaError;
      });
  }
